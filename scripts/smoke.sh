...
PAGE="$(curl -s -b "$NJAR" "$BASE/c/general")"
echo "$PAGE" | grep_all -q 'action="/c/general/mute"' || fail "a room has no bell to mute it"
echo "$PAGE" | grep_all -q 'aria-pressed="true"' || fail "the bell does not show a room muted from the account page"
echo "$PAGE" | grep_all -q 'class=" *[a-z ]*muted-room' || fail "a muted room is not marked in the sidebar"
[ "$(curl -s -b "$NJAR" -o /dev/null -w '%{http_code}' -d "csrf=$NCSRF&muted=0" "$BASE/c/general/mute")" = "303" ] || fail "the bell did not unmute"
curl -s -b "$NJAR" "$BASE/c/general" | grep_all -q 'aria-pressed="true"' && fail "unmuting from the bell did not stick"
ok "a room's bell mutes and unmutes it, and the sidebar marks it"
curl -s -b "$NJAR" -o /dev/null -d "csrf=$NCSRF&level=direct&quiet=1&quiet_start=22:30&quiet_end=06:45&tz=Europe/Paris" "$BASE/account/notifications"
PAGE="$(curl -s -b "$NJAR" "$BASE/account")"
echo "$PAGE" | grep_all -q 'name="quiet" value="1" checked' || fail "quiet hours were not turned on"
echo "$PAGE" | grep_all -q 'value="22:30"' && echo "$PAGE" | grep_all -q 'value="06:45"' && echo "$PAGE" | grep_all -q 'value="Europe/Paris"' || fail "quiet hours were not saved as given"
[ "$(curl -s -b "$NJAR" -o /dev/null -w '%{http_code}' -d "csrf=$NCSRF&level=direct&quiet=1&quiet_start=07:00&quiet_end=07:00" "$BASE/account/notifications")" = "400" ] || fail "quiet hours of no length were accepted"
ok "quiet hours are saved, in the given time zone, and an empty stretch is refused"
