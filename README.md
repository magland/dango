...
- Notifications have no quiet hours yet, and whether someone is reading is judged by whether a page of theirs showing the room is visible, not by whether they are at the screen: a room left open on an unattended desktop keeps their phone quiet about that room. Muting is set from the Account page rather than from the room itself, and notification settings are not yet in the CLI or the JSON API.
